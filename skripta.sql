...
('Dario', 'Romic', 'konobar', 'dario.romic@edunova.hr','$2y$10$U5ykDrG02jI187wbGXuIGucm8ta1kPWVhCgMdLqSzI7y54eaL/kCW'),
('Igor', 'Ivankov', 'konobar', 'igor.ivankov@edunova.hr','$2y$10$U5ykDrG02jI187wbGXuIGucm8ta1kPWVhCgMdLqSzI7y54eaL/kCW'),
('Robert', 'Maric', 'konobar', 'robert.maric@edunova.hr','$2y$10$U5ykDrG02jI187wbGXuIGucm8ta1kPWVhCgMdLqSzI7y54eaL/kCW'),
('Davsifra', 'stetic', 'konobar', 'davsifra.stetic@edunova.hr', '$2y$10$U5ykDrG02jI187wbGXuIGucm8ta1kPWVhCgMdLqSzI7y54eaL/kCW');


insert into korisnik (ime, prezime, status, lozinka, email) values
('Ivano', 'Ivicic', 'kuhar', '$2y$10$U5ykDrG02jI187wbGXuIGucm8ta1kPWVhCgMdLqSzI7y54eaL/kCW', 'ivano.ivicic@edunova.hr'),
...
CREATE TABLE narudzba (
  ...
  cijena	  	decimal(10,2) NOT NULL DEFAULT '0.00',
  stol		 	varchar(5) not null
); 

CREATE TABLE narudzba_ponuda (
  narudzba_sifra        int(11) NOT null,
  ponuda_sifra        int(11) NOT null,
  kolicina  int(11) NOT null default 1
);

select * from narudzba;
select * from narudzba_ponuda;
delete from narudzba_ponuda where narudzba_sifra>0;
delete from narudzba where sifra>0;

select * from ponuda;
alter table narudzba_ponuda add foreign key (narudzba_sifra) references narudzba(sifra);
alter table narudzba_ponuda add foreign key (ponuda_sifra) references ponuda(sifra);

alter table gost add foreign key (sifra) references korisnik(sifra);
alter table djelatnik add foreign key (sifra) references korisnik(sifra);

alter table narudzba_gost add foreign key (sifra_narudzbe) references narudzba(sifra);
alter table narudzba_gost add foreign key (sifra_gosta) references gost(sifra);
...
#alter table ponuda add foreign key (sifra) references narudzba(sifra);
#alter table ponuda add foreign key (sifra) references ponuda(sifra);
...
('Teleci rizoto s pestom od spinata', 'Glavno jelo 3'),
('Tocada od liganja ', 'Riba'),
('Morski pas na trscanski', 'Riba'),
('Bakalar na portugalski', 'Riba'),
('Fishburger', 'Riba'),
('Njoke na maslacu ', 'Prilog'),
('Peceni krumpir s ruzmarinom ', 'Prilog'),
('Pirjana riza ', 'Prilog'),
('Ricet krem riza s kukuruzom ', 'Prilog'),
('Pire od buce', 'Prilog'),
('Zelena', 'Salata'),
('Meksicka', 'Salata'),
('Mjesana', 'Salata'),
...
#select * from narudzba;
#SELECT distinct length(ime) from korisnik; 
